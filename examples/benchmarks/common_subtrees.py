...
from networkx.algorithms.minors.tree_isomorphism import maximum_common_ordered_subtree_isomorphism


def find_shared_structure(tree1, tree2, style, impl='auto', item_type='auto'):
    if style == 'embedding':
        s1, s2 = maximum_common_ordered_tree_embedding(tree1, tree2, impl=impl,
                                                       item_type=item_type)
        value = None
...
def benchmark():
    ...
    def make_grid_from_basis(basis):
        return [dict(zip(basis.keys(), vals))
...
def expand_inner(graph, p_expand, inner_expand_size, rng, base):
    ...
    for source in sources:
        ordered_edges += [
            (u, v) for u, v, etype in list(nx.dfs_labeled_edges(graph, source=source))
